import type { ChatMessage, DeveloperState } from '../types';
import { makeId } from '../lib/id';

const QUESTION_BANK = [
  (s: DeveloperState) => `"${short(s.targetCustomer)}" 고객군 중에서도 가장 먼저 돈을 낼 사람은 누구인가요? 세그먼트를 더 좁혀볼 수 있을까요?`,
  (s: DeveloperState) => `"${short(s.userProblem)}" 문제를 실제로 겪는 사람을 몇 명이나 인터뷰해 보셨나요? 근거 자료가 있다면 함께 정리해보죠.`,
  (s: DeveloperState) => `"${short(s.solution)}" 방식 외에 사용자가 지금 쓰고 있는 대안(경쟁 서비스, 수작업 등)은 무엇인가요?`,
  () => `이 아이디어가 실패한다면 가장 가능성 높은 이유는 무엇일까요? 핵심 가정 중 가장 불확실한 것을 짚어볼까요?`,
  (s: DeveloperState) => `"${short(s.evidence) || '보유 근거'}"를 조금 더 구체적인 수치로 표현할 수 있을까요? (예: 설문 응답 수, 사전 신청자 수)`,
];
...
const GAP_BANK = [
  ...
  (s: DeveloperState) => `'${short(s.currentConcerns) || '현재 고민'}' 항목이 비어 있어요. 지금 가장 걸리는 부분을 적어주시면 그 지점부터 함께 점검하겠습니다.`,
  () => `실행 계획 쪽 정보가 아직 부족합니다. 첫 3개월 안에 무엇을 검증할지 간단히 적어볼까요?`,
];

const ALT_BANK = [
  (s: DeveloperState) => `대안으로, "${short(s.solution)}"을(를) 더 작은 범위(MVP)로 축소해 먼저 검증하는 방법도 있습니다. 어떻게 생각하시나요?`,
  () => `가격을 낮춘 무료 체험판으로 먼저 반응을 보는 방법과, 처음부터 유료로 소수에게 판매해보는 방법 중 어느 쪽이 지금 상황에 맞을까요?`,
  (s: DeveloperState) => `"${short(s.targetCustomer)}" 대신 조금 더 좁은 초기 고객군으로 시작하면 검증 속도를 높일 수 있습니다. 후보를 함께 좁혀볼까요?`,
];

const SUGGESTION_BANK = [
  (s: DeveloperState) => `문장을 조금 더 명확하게 다듬어보면: "${short(s.userProblem)} 때문에 시간을 낭비하는 사람들에게, ${short(s.solution)}로 대안을 제공합니다." 이런 식으로 정리할 수 있어요.`,
  () => `점검 기준 탭에서 '문제 적합성'과 '고객 정의' 항목을 먼저 채워보시는 것을 추천합니다. 근거가 쌓일수록 다음 단계 문서 품질이 좋아집니다.`,
  (s: DeveloperState) => `"${short(s.evidence)}"를 뒷받침할 추가 자료(설문, 사전 신청, 인터뷰 메모)가 있다면 첨부해두면 Planner 단계에서 설득력이 높아집니다.`,
];
...
function pickBank(bank: Array<(s: DeveloperState) => string>, state: DeveloperState, seed: number): string {
...
export function openingMessage(state: DeveloperState): ChatMessage {
  const parts: string[] = [];
...
export function generateAiReply(state: DeveloperState, _userMessage: string): ChatMessage {
  turnCounter += 1;
